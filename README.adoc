...
[source,shell]
[redhat-sso 534]$ java -jar target/sso-client.jar -help
...
The only required argument is the --app argument. This needs to math up to the name of the service endpoint as known by OSO. You
can get a list of the using the `oc get routes` command, for which a sample output where all 3 secured endpoints and the
RH SSO server have been deployed:
...
Choose a name from among the following, depending on what you have deployed:
