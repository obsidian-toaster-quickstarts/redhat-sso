...
=== The secured-booster-endpoint Client
The secured-booster-endpoint is a bearer-only type of client that requires a booster-admin role for accessing the
...
alice has been granted the booster-admin role. We will see that the Vert.X secured endpoint will look to the
...
<4> The resource_access.secured-booster-endpoint.roles object contains the roles granted to alice for the secured-booster-endpoint
resource.
